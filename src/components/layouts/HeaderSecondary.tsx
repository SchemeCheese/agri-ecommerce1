...
import Image from 'next/image'; // Dùng Image tối ưu hơn
      ...
      <Container className="flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-6">
          ...
          <button
            ...
            className="text-sm font-semibold text-gray-700 hover:text-green-600 uppercase"
          >
            {currentLanguage}
          </button>
          ...
          {mounted && user ? (
            <div className="relative group">
                <Link href={user.role === 'seller' ? '/dashboard' : '/profile'} className="flex items-center space-x-2 pl-2 border-l border-gray-200 text-gray-700 hover:text-green-600 cursor-pointer">
                  <div className="w-8 h-8 rounded-full bg-green-100 text-green-700 flex items-center justify-center font-bold text-sm border border-green-200 overflow-hidden relative">
                    {user.avatar ? (
